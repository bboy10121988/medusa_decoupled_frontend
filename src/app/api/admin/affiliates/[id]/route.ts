...
export async function GET(
    ...
    try {
        ...
        if (!res.ok) {
            ...
            console.error('[Admin Detail Proxy] Backend Error:', res.status, errorText)
            return NextResponse.json({ error: 'Failed to fetch affiliate' }, { status: res.status })
        }

        const data = await res.json()
        return NextResponse.json(data)
    } catch (error) {
        console.error('[Admin Detail Proxy] Error:', error)
