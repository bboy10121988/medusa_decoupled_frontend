  ...
  try {
    ...
    try {
      ...
      if (customerResponse?.customer) {
        ...
        // 🔥 修正：區分新用戶和舊用戶的 email 處理邏輯
        const isNewUser = !payload?.actor_id
        const customerEmail = customerResponse.customer.email
        const needsRealEmail = customerEmail?.startsWith('debug-') || 
                              customerEmail?.startsWith('temp-google-') ||
                              email.startsWith('temp-google-')
        
        if (needsRealEmail && !isNewUser) {
          console.log("🔄 舊用戶：從資料庫provider_identity表獲取真實 Google email...")
